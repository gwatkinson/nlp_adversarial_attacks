...
@click.option('--data-folder', '-o', default='data', help='Folder in which to save the data.')
@click.option('--quiet/--verbose', '-q/-v', default=False, help='Flag to not show the download bar.')
@click.option('--fuzzy/--no-fuzzy', '-f/-e', default=True, help='Flag to extract id from fuzzy url.')
def main(url, data_folder, quiet, fuzzy):
    ...
    if not quiet: click.echo("Extracting the data...")
    extract_data(data_path, tar_file)

    if not quiet: click.echo("Cleaning up...")
    clean_up(tar_file)
